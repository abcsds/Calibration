...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='G:\\ULSA\\calibration\\calibration_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    text_instructions = visual.TextStim(win=win, name='text_instructions',
        text='Observa los distintos puntos en la pantalla para calibrar el eyetracker.\n\n[Presiona la barra espaciadora para continuar]',
        font='Open Sans',
    ...
    point_of_interest = visual.ShapeStim(
        win=win, name='point_of_interest',
    ...
    # keep track of which components have finished
    calibrationComponents = [circle_cal_1, circle_cal_2, circle_cal_3, circle_cal_4]
    ...
    while continueRoutine and routineTimer.getTime() < 4.3:
        ...
        if circle_cal_1.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > circle_cal_1.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if circle_cal_2.status == NOT_STARTED and tThisFlip >= 1.1-frameTolerance:
            # keep track of start time/frame for later
        ...
        if circle_cal_2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > circle_cal_2.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if circle_cal_3.status == NOT_STARTED and tThisFlip >= 2.2-frameTolerance:
            # keep track of start time/frame for later
        ...
        if circle_cal_3.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > circle_cal_3.tStartRefresh + 1-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if circle_cal_4.status == NOT_STARTED and tThisFlip >= 3.3-frameTolerance:
            # keep track of start time/frame for later
        ...
        if circle_cal_4.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > circle_cal_4.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
    ...
    # keep track of which components have finished
    feedbackComponents = [point_of_interest]
    ...
    while continueRoutine:
        ...
        # *point_of_interest* updates
        
        # if point_of_interest is starting this frame...
        if point_of_interest.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            point_of_interest.frameNStart = frameN  # exact frame index
            point_of_interest.tStart = t  # local t and not account for scr refresh
            point_of_interest.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(point_of_interest, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'point_of_interest.started')
            # update status
            point_of_interest.status = STARTED
            point_of_interest.setAutoDraw(True)
        
        # if point_of_interest is active this frame...
        if point_of_interest.status == STARTED:
            # update params
            pass
